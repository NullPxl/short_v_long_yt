"""
Collect longitudinal YouTube Shorts stats (views, likes, comments) into CSV.

Uses YouTube API. Supports multiple channels. Only collects shorts (duration <= 60s)
...
# Videos with duration <= 60 seconds are shorts; we include only these
...
class YouTubeShortsCollector:
    ...
    def discover_shorts_since(
        self,
        channel_id: str,
        since: dt.datetime,
        max_pages: int = 10,
    ) -> List[ShortRecord]:
        """Discover shorts posted after `since` for the given channel."""
        ...
        return self.fetch_short_records(video_ids, channel_id)

    def fetch_short_records(
        self,
    ...
    def collect_once(
        ...
        all_records: List[ShortRecord] = []
        for channel_id in channel_ids:
            records = self.discover_shorts_since(channel_id, since)
            all_records.extend(records)

        seen: Dict[str, ShortRecord] = {}
        ...
        return {"shorts": len(seen), "rows_written": count}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Collect YouTube Shorts stats (views/likes/comments) into CSV. "
        "Checks for new shorts after a specified datetime. Polls every 30 seconds."
    )
    parser.add_argument(
        "--api-key",
    ...
    parser.add_argument(
        ...
        help="Only collect shorts published after this datetime (ISO8601, e.g. 2025-03-01T00:00:00Z)",
    )
    parser.add_argument(
        "--output",
        default="shorts_stats.csv",
...
def main() -> None:
    ...
    collector = YouTubeShortsCollector(api_key=args.api_key, output_csv=args.output)
    ...
    logging.info(
        "Starting shorts collector: channels=%s, since=%s, poll=%ss, output=%s",
        args.channels,
        args.since,
        args.poll_seconds,
        args.output,
